#include "graph.h"
 ...
 void Graph<V, E>::bellman_ford(const V& data) {
 	for (int i = 0; i < vertices.size(); i++) vertices[i].bellman_len = 1e9; //INT32_MAX; ïëîõàÿ èäåÿ áðàòü ìàêñèìàëüíûé int â êà÷åñòâå áåñêîíå÷íîñòè, òê èä¸ò ïðè +(>0) ïåðåõîä â -
 ...
 vector3d Graph<V, E>::warshall_floyd() {
 	int n = vertices.size();
 	vector3d A(n + 1, vector<vector<int>>(n + 1, vector<int>(n + 1, 1000000000))); //çàïîëíåí ñíà÷àëà 10å9
 	for (int v = 1; v <= n; v++) {
 		for (int w = 1; w <= n; w++) {
